...
/* External boot functions */
extern image_type_t detect_image_type(unsigned char *addr);
extern int boot_nuttx(image_info_t *image);
extern int boot_linux(image_info_t *image);
extern int boot_image_setup(unsigned char *addr, unsigned int *entry);
...
_boot:
	...
	{
		image_type_t img_type = detect_image_type(image.dest);

		if (img_type == IMAGE_TYPE_NUTTX) {
			info("booting NuttX...\r\n");
			boot_nuttx(&image);
			fatal("NuttX boot failed\r\n");
		} else if (img_type == IMAGE_TYPE_LINUX) {
			info("booting Linux (EGON header)...\r\n");
			/* Skip EGON header for Linux */
			image.dest += EGON_HEADER_SIZE;
			boot_linux(&image);
			fatal("Linux boot failed\r\n");
		} else {
			/* Fallback: try Linux zImage detection */
			info("booting Linux (zImage)...\r\n");
			boot_linux(&image);
			fatal("boot failed\r\n");
		}
	}
